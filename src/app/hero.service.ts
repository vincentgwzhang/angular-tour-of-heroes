...
export class HeroService {
  ...
  private heroURL = 'http://127.0.0.1:8080/api/hero';
  ...
  getHeroes(): Observable<Hero[]> {
    return this.http.get<Hero[]>(this.heroesUrl)
      .pipe(
        catchError(this.handleError<Hero[]>('getHeroes', []))//catchError() 操作符会拦截失败的 Observable。 它把错误对象传给错误处理器，错误处理器会处理这个错误。
      );
  }
  ...
  updateHero(hero: Hero): Observable<any> {
    return this.http.put(this.heroURL, hero, this.httpOptions).pipe(
      tap((updateHero: Hero) => this.messageService.addMessage(`updated hero id=${updateHero.id}`)),
  ...
  addHero(hero: Hero): Observable<Hero> {
    return this.http.post<Hero>(this.heroURL, hero, this.httpOptions).pipe(
      tap((newHero: Hero) => this.messageService.addMessage(`added hero id=${newHero.id}`)),
  ...
  deleteHero(hero: Hero | number): Observable<Hero> {
    ...
    return this.http.delete<Hero>(url, this.httpOptions).pipe(
      tap(() => this.messageService.addMessage(`deleted hero id=${id}`)),
